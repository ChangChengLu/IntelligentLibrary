...
const BookBugPage: React.FC = () => {
  ...
  const computerPrice = async () => {
    const totalMoney = await computerTotalPriceUsingPOST(orderReq);
    const discountMoney = await computerDiscountPriceUsingPOST(orderReq);
    setTotalMoney(totalMoney.data)
    setDiscountMoney(discountMoney.data)
  }
  ...
  const handlerClick = async () => {
    await bookOrderBorrowUsingPOST(orderReq)
  }
  ...
  const handlerCancel = (item: API.BookInfo) => {
    ...
    const newOrderReq = {
      ...orderReq,
      orderInfoList: orderReq.orderInfoList?.filter((item) => item !== orderInfo)
    };
    setOrderReq(newOrderReq);
  }
          ...
          <List.Item key={item.id}>
            <Card
              ...
              actions={[
                ...
                <Button onClick={() => handlerBuy(item)}> 借阅 </Button>,
                // eslint-disable-next-line react/jsx-key
                <Button onClick={() => handlerCancel(item)}> 取消 </Button>,
                // eslint-disable-next-line react/jsx-key
                <Link to={`/book/info?bookId=${item.id}`} target={'_blank'}> 详情 </Link>,
              ]}
            >
              <List.Item.Meta
                title={item.bookName}
